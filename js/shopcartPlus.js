...
$(function() {
    //把三个类型的input分别先获取
    var $theadIuput = $('thead input[type=checkbox]'); //表头中的全选框
    ...
    */
    $totalPriceInput.change(function() {
        var checkState = $(this).prop('checked'); //获取计算机总价中的全选状态
        $theadIuput.prop('checked', checkState); //赋值给表头全选框
    ...
    $tbodyInputs.change(function() { //给表格总单选框绑定事件
        ...
        $tbodyInputs.each(function(index, input) { //循环表格input
            ...
            if (checkState === false) { //如果有一个等于false
                flag = false; //标杆变为false（全选的状态为false）
            }
        })
        $theadIuput.prop('checked', flag); //把状态赋值给头部全选框
    ...
    */
    function subTotal(dom, count) {
        ...
        dom.closest('tr').find('.subprice').text(subTotalPrice.toFixed(2)) //把小计的结果渲染到对应的位置 保留2位数
    }
    ...
    */
    ...
    $('.deleteChecked').click(function() {
            $('tbody input[type=checkbox]').each(function() { //获取表格中的选择框
                var checkState = $(this).prop('checked'); //获取选择状态
                if (checkState) { //如果是true
                    $(this).closest('tr').remove(); //干掉自己
                }
            })
